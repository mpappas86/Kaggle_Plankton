...
from Features import shape_features as sf

##NOTE:: CURRENTLY THIS IS CONFIGURED TO INCLUDE ALL PIXELS AND ALL FEATURES CALCULATED ON THE FULL IMAGE.
...
for cls in classcounts:
    ...
    for filename in os.walk(filepath).next()[2]:
        ...
        f1 = sf.getMinorMajorRatio(image)
        f2 = sf.getHorizontalSymmetryFeature(image)
        f3 = sf.getVerticalSymmetryFeature(image)
        f4 = sf.getCircularSymmetryFeature(image)
        f5 = sf.getPercentPixelsAboveAverage(image)
        featureVals = np.array([f1, f2, f3, f4, f5])

        #rawdata.append(np.concatenate((featureVals, label)))
        rawdata.append(np.concatenate((rimage.reshape((image_size)), featureVals, label)))
rawdata = np.array(rawdata)
np.random.shuffle(rawdata)
rawdata = rawdata.T
data = rawdata[:num_features,:]
labels = rawdata[num_features:,:]
